...
def plot_csvs(csv_files):
    ...
    for csv_file in csv_files:
        ...
        if "MatrixSize" not in df.columns or "Bandwidth_GBps" not in df.columns:
            print(f"Skipping {csv_file}, missing required columns")
            continue
        label = input(f"Enter label for {Path(csv_file).name}: ")
        plt.plot(df["MatrixSize"], df["Bandwidth_GBps"], marker='o', label=label)
    ...
    ax = plt.gca()
    TICK_INTERVAL = int(input("Enter x-axis tick interval (e.g., 1000): "))
